...
SRCS		= DE0_CV.nsl ADD.nsl ADDS.nsl CALC_COMP.nsl CALC_TARGET.nsl CALC_VOUT.nsl DS_DAC.nsl MUL.nsl MULS_FIXP.nsl PLAYTONE_ST.nsl SOUND_CNT.nsl
VFILES 		= $(SRCS:%.nsl=%.v) 
LIBS		=
RESULT		= result.txt

SCFILES		=$(SRCS:%.nsl=%.sc)
INCDIR		=$(SYSTEMC_INCLUDE_DIRS) -I$(DEBUGDIR)
...
$(PROJECT).exe:$(PROJECT)_main.cpp $(SCFILES)
	$(CXX) $(CXXFLAGS) $(INCDIR) $(LIBDIR) -o $@ $(LIBSC) $<
